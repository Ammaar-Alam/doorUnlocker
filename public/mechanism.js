...
async function createMechanism() {
  ...
  const parts = [
    ...
    { id: 'motor', name: 'Gearmotor', description: 'The BRINGSMART worm gearmotor trades speed for pulling torque through a right-angle gearbox. The calibrated opening stroke takes up the line; a shorter, gentler reverse stroke releases it.', specs: '24 V motor · 0.97 s open · 0.60 s release', group: motor, point: [-10, -61, 8], x: .06, y: .56 },
  ...
  function animate(open, start = performance.now()) {
    motion = { from: position, to: open ? 1 : 0, opening: open, start, duration: open ? 970 : 600 };
