...
void onDoorOpenChange()  {
  ...
  if (doorOpen) {
      ...
      analogWrite(ENA, 100); // ADJUSTABLE speed; found to work well w/o allowing motor to over-tangle in opposite direction
      digitalWrite(RED_LED, LOW);  // turn off RED led
      digitalWrite(GREEN_LED, HIGH); // turn on GREEN led
      delay(motorRunTime - 1350); // runtime less than OPEN door to prevent string from overtangling in opposite direction
      digitalWrite(IN1, LOW);
      digitalWrite(IN2, LOW);
      analogWrite(ENA, 0); // stop the motor
  }
