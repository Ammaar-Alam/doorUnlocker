...
Opening runs a calibrated 970 ms stroke; releasing runs for 600 ms. Each Open or Close received while idle runs a full stroke, including repeated commands in the same direction. Calibrate the timings for your own motor and handle.
...
The [iOS app](ios/README.md) provides the website interface on iOS 26+, Bluetooth reconnection, and live diagnostics, with native controls available as a fallback. Open `ios/DoorOpener.xcodeproj` in Xcode. It supports iPhone on iOS 18 or later.
